 ...
 * @Description MapperBuilder助手，负责加载处理解析出来的数据
 */
public class MapperBuilderAssistant extends BaseBuilder {
    ...
    /**
     * 获取缓存 步骤如下： 1. 根据命名空间从全局配置的缓存对象中查找该命名空间对应的缓存 2. 判断缓存是否为空，不空返回，空的话抛出未完成异常（IncompleteElementException）
     * 3. 上层捕获IncompleteElementException，将缓存引用解析器存储到全局未完成缓存引用列表，等待xmlMapperBuilder.parsePendingCacheRefs()去再次解析
     */
    public Cache useCacheRef(String namespace) {
        ...
        try {
            ...
            // 根据命名空间从全局配置的缓存对象中查找该命名空间对应的缓存
            Cache cache = configuration.getCache(namespace);
            // 如果缓存不存在，则抛出未完成异常（IncompleteElementException）
            if (cache == null) {
    ...
    /**
     * 获取一个新的缓存 根据传入的配置参数创建一个新的缓存构建对象，并通过其创建一个新的缓存
     */
